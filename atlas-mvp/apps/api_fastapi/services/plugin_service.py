...
class PluginService:
    ...
    def _load_manifest(self, manifest_path: Path) -> Optional[Plugin]:
        ...
        module_path = manifest_path.parent / manifest.get("entrypoint", "example_tool.py")
        ...
        if not state:
            ...
            self.session.flush()
        return Plugin(name=name, description=description, module_path=module_path, enabled=enabled)
    ...
    def run_plugin(self, name: str, payload: Dict) -> Dict:
        ...
        if not hasattr(module, "run"):
            raise HTTPException(status_code=500, detail="Plugin missing run() function")
