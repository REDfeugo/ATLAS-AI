...
def main() -> None:
    ...
    st.write(
        ...
        your machine. Provide an `OPENAI_API_KEY` to unlock cloud fallback when needed.
        """
    )
    ...
    st.markdown(
        ...
        1. **Chat** with the local LLM (or slash commands like `/task add "Review"`).
        2. Capture **tasks** and **notes**, then search them semantically.
        3. Use the **Voice & Control** page to transcribe audio and preview planner steps.
        4. Drive automations from the **Command Line** page with dry-run previews.
        5. Extend Atlas via **plugins** and review execution history in the audit log.
        """
    )
