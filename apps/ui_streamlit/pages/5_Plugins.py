...
def main() -> None:
    ...
    st.write(
        ...
        Each plugin defines a `manifest.yaml` describing its name, description, permission tier,
        and the path to a `run(payload)` function. Enable or disable them here!
        """
    )
    ...
    if not available:
        st.markdown(
            "<div class='atlas-empty'>No plugins discovered yet. See docs/04_PLUGINS_AND_PERMISSIONS.md to add one.</div>",
