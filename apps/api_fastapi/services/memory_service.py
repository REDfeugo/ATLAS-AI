"""Memory operations for chats, notes, document chunks, and embeddings."""
...
class MemoryService:
    """Persist and query chats, notes, tasks, and semantic embeddings."""
    ...
    def upsert_document_chunk(
        self, source_path: str, chunk_index: int, content: str
    ) -> models.DocumentChunk:
        """Store a document chunk and its embedding."""

        stmt = select(models.DocumentChunk).where(
            models.DocumentChunk.source_path == source_path,
            models.DocumentChunk.chunk_index == chunk_index,
        )
        chunk = self.session.scalars(stmt).one_or_none()
        if chunk:
            chunk.content = content
        else:
            chunk = models.DocumentChunk(
                source_path=source_path, chunk_index=chunk_index, content=content
            )
            self.session.add(chunk)
            self.session.flush()
        self._store_embedding("document", chunk.id, content)
        return chunk

    # Search ----------------------------------------------------------------
    def semantic_search(self, query: str, top_k: int = 5) -> List[dict]:
        """Search notes, tasks, and documents by semantic similarity."""

        query_vec = self.embed_service.embed([query])[0]
        stmt = select(models.Embedding)
        matches: List[tuple[models.Embedding, float]] = []
        ...
        for embedding, score in matches[:top_k]:
            ...
            elif embedding.item_type == "document":
                chunk = self.session.get(models.DocumentChunk, embedding.item_id)
                if chunk:
                    results.append(
                        {
                            "type": "document",
